...
<div class="p-4 sm:p-8">
    <div class="bg-white rounded-3xl shadow-xl p-6 sm:p-10 border-t-8 border-teal-500 animate-slide-in-up">
        <div class="flex flex-col md:flex-row justify-between items-center mb-8">
            ...
            <a href="{{ route('users.create') }}" class="w-full md:w-auto bg-teal-500 text-white px-6 py-3 rounded-full font-semibold hover:bg-teal-600 transition-all duration-300 transform hover:scale-105 shadow-md flex items-center justify-center space-x-2 animate-slide-in-up" style="animation-delay: 0.4s;">
            ...
            <table class="min-w-full bg-white table-auto rounded-xl overflow-hidden border-collapse">
                <thead class="bg-gray-100 border-b border-gray-200 sticky top-0 z-10">
                    <tr class="text-gray-600 uppercase text-sm leading-normal">
                        ...
                        <th class="py-4 px-3 sm:px-6 text-left font-bold">Nama</th>
                        <th class="py-4 px-3 sm:px-6 text-left font-bold">Email</th>
                        <th class="py-4 px-3 sm:px-6 text-left font-bold">Role</th>
                        <th class="py-4 px-3 sm:px-6 text-center font-bold">Aksi</th>
                    </tr>
                </thead>
                <tbody class="text-gray-800 text-sm sm:text-base font-light">
                    @forelse($users as $item)
                        <tr class="border-b border-gray-100 hover:bg-teal-50 transition duration-200 transform hover:scale-[1.01] hover:shadow-md">
                            ...
                            <td class="py-3 px-3 sm:px-6 text-left">{{ $item->name }}</td>
                            <td class="py-3 px-3 sm:px-6 text-left">{{ $item->email }}</td>
                                ...
                                @php
                                    ...
                                    switch ($role) {
                                        case 'admin':
                                            ...
                                            $icon = '<i class="fas fa-crown text-yellow-300 mr-2"></i>';
                                            break;
                                        case 'guru':
                                            $badgeClass = 'bg-teal-600 text-white font-semibold';
                                            $icon = '<i class="fas fa-user-tie text-white mr-2"></i>';
                                            break;
                                        default:
                                            $badgeClass = 'bg-gray-400 text-white font-normal';
                                            $icon = '';
                                            break;
                                    }
                                @endphp
                                <span class="py-1.5 px-3 rounded-full text-sm flex items-center {{ $badgeClass }}">
                                    {!! $icon !!}
                                    {{ $roleText }}
                                </span>
                            </td>
                            <td class="py-3 px-6 text-center">
                                <div class="flex item-center justify-center space-x-2 aksi-mobile-stack">
                                    <a href="{{ route('users.edit', $item->id) }}" class="h-8 flex items-center justify-center px-4 py-2 rounded-full bg-yellow-100 text-yellow-600 hover:bg-yellow-300 hover:text-yellow-800 transition duration-300 font-semibold text-sm transform hover:scale-105">
                                        Edit
                                    </a>
                                    @if(Auth::user()->id !== $item->id)
                                        <form action="{{ route('users.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus user ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="h-8 flex items-center justify-center px-4 py-2 rounded-full bg-red-100 text-red-600 hover:bg-red-300 hover:text-red-800 transition duration-300 font-semibold text-sm transform hover:scale-105">
                                                Hapus
                                            </button>
                                        </form>
                                    @endif
                        ...
                        <tr>
                            <td colspan="5" class="py-10 text-center text-gray-500 animate-slide-in-up" style="animation-delay: 1s;">
                                <div class="flex flex-col items-center justify-center">
                                    <i class="fas fa-users-cog text-5xl text-gray-300 mb-4 animate-bounce-subtle"></i>
                                    <p class="text-lg font-semibold text-gray-600">Tidak ada data user.</p>
                                    <p class="text-gray-400 mt-1">Silakan tambahkan user baru untuk memulai.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
